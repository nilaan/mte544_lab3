...
void bresenham(double x0, double y0, double x1, double y1, std::vector<double>& x, std::vector<double>& y)
{
...
{
	int x0 = nodesX[i]/mapRes;
	int x1 = nodesX[j]/mapRes;
	int y0 = nodesY[i]/mapRes;
	int y1 = nodesY[j]/mapRes;
	uint16_t ind = 0;
	long int size = 0;
	std::vector<int> indX;
	std::vector<int> indY;
	...
	bresenham(x0, y0, x1, y0, indX, indY);
